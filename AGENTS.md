...
`homearo-promotional-images` contains Polish instructions for narrative Homearo promotional graphics and captions, including prominent typography, both mandatory promotional messages, and a uniform operator signature. Each publication uses a separately generated image and distinct single-sentence caption in the format `www.homearo.pl (@profile) - Description.`, with the verified mention omitted when unavailable. Emoji are optional on Instagram/Facebook and excluded on LinkedIn; supported hashtags are relevant and limited to five. This also applies to separate Homearo page and Mateusz Adam Katana profile variants. Its `references/instagram-style.md` maps composition directions from the historical Instagram series; the source images belong to the Homearo workspace, not this repository. Save generated campaign artifacts in the target project rather than the skill directory.
