...
`homearo-promotional-images` contains Polish instructions for narrative Homearo promotional graphics and captions, including prominent typography, both mandatory promotional messages, and a uniform operator signature. Each publication uses a separately generated image and distinct caption beginning with the portal address on the same line: `www.homearo.pl` for Instagram and Facebook, `https://homearo.pl/pl/` for LinkedIn, including separate Homearo page and Mateusz Adam Katana profile variants. Its `references/instagram-style.md` maps composition directions from the historical Instagram series; the source images belong to the Homearo workspace, not this repository. Save generated campaign artifacts in the target project rather than the skill directory.
